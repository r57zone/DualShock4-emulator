...
3. При необходимости уменьшите общую чувствительность (параметр Sens в разделе `Motion`, где `100` — 100% чувствительность), в конфигурационном файле.
4. При необходимости уменьшите чувствительность отдельных датчиков (`AccelSens` и `GyroSense` в разделе `Motion`, где `100` — 100% чувствительность), в конфигурационном файле.
5. При необходимости инвертируйте оси (параметры `InverseX`, `InverseY` и `InverseZ`, в секции `Motion`, где `1` включает инверсию, а `0` выключает).
6. Измените ориентацию телефона (параметр `Ориентация` в разделе `Motion`, где `1` — альбомная, а `0` — портретная).
